...
class SetupForm(FlaskForm):
    ...
    another_mail = StringField("User уже зареган по другой почте (обязательно нужен {link}):",
                               default="Ты уже зарегистрирован по другой почте.")
    register = StringField("Проверяем почту:",
    ...
    check = StringField("Пользователь не прошел /check проверку:",
...
class Server(Flask):
    ...
    def setup(self):
        form = SetupForm()
        # form.greetings.data = self._setuper.greetings
        # form.same_mail.data = self._setuper.same_mail
        # form.another_mail.data = self._setuper.another_mail
        # form.register.data = self._setuper.register
        # form.no_mail.data = self._setuper.no_mail
        # form.oauth_good.data = self._setuper.oauth_good
        # form.oauth_bad.data = self._setuper.oauth_bad
        # form.kick.data = self._setuper.kick
        # form.rights.data = self._setuper.rights
        # form.went_wrong.data = self._setuper.went_wrong
        # form.ban.data = self._setuper.ban
        # form.unban.data = self._setuper.unban
        # form.ignore.data = self._setuper.ignore
        # form.banned.data = self._setuper.banned
        # form.deadline.data = self._setuper.deadline
        # form.password.data = ""
        if form.validate_on_submit():
            print(str(form.password.data), SETUP_PASSWORD)
